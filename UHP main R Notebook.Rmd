...
```{r}
# Read and preprocess Covid Stringtency data
covid_data <- read_csv("OxCGRT_simplified_Covid stringency.csv") %>%
  mutate(
    Date = ymd(Date),
    Year = year(Date),
    RegionName = ifelse(RegionName == "Washington DC", "District of Columbia", RegionName)
  ) %>%
...
## 2.2 Preprocess and Merge Covid contigency

**State Name to STATEQ Code Mapping**
...
**Filter for mean and max stringency over 2020 and 2021**

```{r}
# Mean and max stringency
mean_stringency <- covid_data %>% 
    left_join(state_map, by = "RegionName") %>% #Map state number to state name 
    group_by(STATEQ, RegionName, Year) %>%
    summarise(StringencyIndex = mean(StringencyIndex_Average, na.rm = TRUE), .groups = "drop") %>%
  drop_na(STATEQ)

max_stringency <- covid_data %>% 
  left_join(state_map, by = "RegionName") %>%
  group_by(STATEQ,RegionName, Year) %>%
  summarise(StringencyIndex = max(StringencyIndex_Average, na.rm = TRUE), .groups = "drop") %>%
  drop_na(STATEQ)

# Filter for specific years
mean_stringency_2021 <- mean_stringency %>%
  filter(Year == 2021) %>%
  select(-Year) %>%
  rename(mean_stringency_2021 = StringencyIndex)

max_stringency_2021 <- max_stringency %>%
  filter(Year == 2021) %>%
  select(-Year) %>%
  rename(max_stringency_2021 = StringencyIndex)

mean_stringency_2020 <- mean_stringency %>%
  filter(Year == 2020) %>%
  select(-Year) %>%
  rename(mean_stringency_2020 = StringencyIndex)

max_stringency_2020 <- max_stringency %>%
  filter(Year == 2020) %>%
  rename(max_stringency_2020 = StringencyIndex)
```

**Merge NFCS and Covid stringency datasets**

```{r}
#Merge State and Investor datasets
NFCS_data <- NFCS_data %>% 
  left_join(mean_stringency_2021, by="STATEQ") %>% 
  left_join(max_stringency_2021, by="STATEQ") %>%
  left_join(mean_stringency_2020, by="STATEQ") %>% 
  left_join(max_stringency_2020, by="STATEQ") 

```

## 2.3 Filter key variables
...
key_vars <- c(
  ...
  "source_year",
  # Demographics
  "A50A", "A3Ar_w", "A4A_new_w", "A5_2015", "A9", "A21_2015","A22_2015", "A8", "STATEQ",
  ...
  #Stringency Indexes
  "mean_stringency_2021", "max_stringency_2021",     "mean_stringency_2020", "max_stringency_2020"
)

NFCS_selected <- NFCS_data %>% select(all_of(key_vars))
```

## 2.4 Create new target variable

Create binary risk_behavior variable based on question B10 in which respondent chooses one out of 4 financial risk taking statements

```{r}
NFCS_selected <- NFCS_selected %>% mutate(risk_behavior = case_when(
  B10 %in% c(1,2,3) ~ 1,
  B10 == 4 ~ 0,
  TRUE ~ NA_real_
))

sum(NFCS_selected$risk_behavior==0)/sum(NFCS_selected$risk_behavior)
```

```{r}
#View number of 1s and 0s for risk_behavior variable
sum(NFCS_selected$risk_behavior==0, na.rm = 1)
sum(NFCS_selected$risk_behavior==1, na.rm = 1)
...
## 3.3.1 Financial Knowledge - Objective and Subjective
...
## 3.3.2 Financial Risk & Investment
...
### Transform B10 into a binary variable:

```{r}
# Transform B10 into a binary variable: 
# 1 = Risk taker (original answers 1–3), 0 = Risk hater (original answer 4)

NFCS_selected <- NFCS_selected %>%
  mutate(
    B10_binary = case_when(
      B10 %in% c(1, 2, 3) ~ 1,  # Risk takers
      B10 == 4 ~ 0,            # Risk haters
...
### Rename & Convert Datatype Where Needed

```{r}
# Rename variables
NFCS_selected <- NFCS_selected %>%
  rename(
    Risk_Taker = B10_binary,
    Age_Group = A3Ar_w,
    Gender = A50A,
    Education_Level = A5_2015,
    Income_Bracket = A8,
    Objective_Knowledge = fin_knowledge_score,
    Subjective_Knowledge = G2,
    Investing_Confidence = G21,
    ShortTerm_Investor = G30_1.y
  )
...
NFCS_selected <- NFCS_selected %>%
  mutate(
    ...
    Investing_Confidence = as.factor(Investing_Confidence),
    ShortTerm_Investor = as.factor(ShortTerm_Investor)
  )
```

### Logistic Regression: Mean Stringency 2020

```{r}
model_mean_2020 <- glm(
  Risk_Taker ~ mean_stringency_2020 + Objective_Knowledge + Subjective_Knowledge +
    Investing_Confidence + ShortTerm_Investor + Age_Group + Gender +
    Education_Level + Income_Bracket,
  data = NFCS_selected,
  family = binomial()
)
summary(model_mean_2020)


```

### Logistic Regression: Max Stringency 2020

```{r}
model_max_2020 <- glm(
  Risk_Taker ~ max_stringency_2020 + Objective_Knowledge + Subjective_Knowledge +
    Investing_Confidence + ShortTerm_Investor + Age_Group + Gender +
    Education_Level + Income_Bracket,
  data = NFCS_selected,
  family = binomial()
)
summary(model_max_2020)
```

### **Logistic Regression: Mean Stringency 2021**

```{r}
model_mean_2021 <- glm(
  Risk_Taker ~ mean_stringency_2021 + Objective_Knowledge + Subjective_Knowledge +
    Investing_Confidence + ShortTerm_Investor + Age_Group + Gender +
    Education_Level + Income_Bracket,
  data = NFCS_selected,
  family = binomial()
)
summary(model_mean_2021)
```

### **Logistic Regression: Max Stringency 2021**

```{r}
model_max_2021 <- glm(
  Risk_Taker ~ max_stringency_2021 + Objective_Knowledge + Subjective_Knowledge +
    Investing_Confidence + ShortTerm_Investor + Age_Group + Gender +
    Education_Level + Income_Bracket,
  data = NFCS_selected,
  family = binomial()
)
summary(model_max_2021)
```
...
```{r}
model_mean_2020 <- glm(
  Risk_Taker ~ mean_stringency_2020 + Subjective_Knowledge +
    Investing_Confidence + ShortTerm_Investor + Age_Group + Gender +
    Education_Level + Income_Bracket,
  data = NFCS_selected,
  family = binomial()
